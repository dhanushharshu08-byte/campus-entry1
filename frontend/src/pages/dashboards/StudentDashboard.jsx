...
const StudentDashboard = () => {
  ...
  // Ensure user joins their private socket room
  useEffect(() => {
    if (user?.id) {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      console.warn('Error fetching student dashboard data:', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div>
      {/* Header Banner */}
      <div className="card" style={{ padding: '2rem', marginBottom: '1.75rem', background: 'linear-gradient(135deg, #0f172a 0%, #1e3a5f 100%)', color: '#fff', border: 'none' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', backgroundColor: 'rgba(59, 130, 246, 0.2)', color: '#93c5fd', padding: '0.3rem 0.75rem', borderRadius: '9999px', fontSize: '0.78rem', fontWeight: 600, marginBottom: '0.65rem' }}>
              <span>{COLLEGE_CONFIG.COLLEGE_SHORT_NAME} Student Redressal Desk</span>
            </div>
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            <Link to="/complaints/new" className="btn btn-primary" style={{ padding: '0.7rem 1.3rem', fontSize: '0.9rem' }}>
              <PlusCircle size={16} />
      ...
      {/* Primary Statistics Grid (Real Database Data) */}
      ...
      <div className="card">
        <div className="card-header">
          <div>
            ...
            <p className="card-subtitle">Showing latest tickets submitted by your account</p>
          </div>
                  ...
                  <td colSpan="8" style={{ textAlign: 'center', padding: '3rem 1.5rem', color: 'var(--color-slate-500)' }}>
                    {loading ? (
                      ...
                      <div>
                        ...
                        <Link to="/complaints/new" className="btn btn-primary" style={{ padding: '0.6rem 1.25rem', fontSize: '0.875rem' }}>
                          <PlusCircle size={15} />
                          <span>Report an Issue</span>
                        </Link>
                      </div>
